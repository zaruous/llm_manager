...
`./gradlew build` 정상 통과 (2026-07-03 기준, chroma-db·tag-build-failure 브랜치 병합 후).
...
- 임베딩 재연결(body_hash): `chunks.body_hash` 컬럼(자동 마이그레이션+백필). 변경 감지를
  ...
  임베딩 API 호출 없이 기존 벡터를 재연결한다. `WikiIndexService.planReconcile()`이
  스킵/재연결/재임베딩/삭제 계획을 수립하고, `WikiVectorRepository.relinkChunks()`가
  임시 음수 chunk_no 2-pass 트랜잭션으로 UNIQUE 제약 충돌 없이 번호를 이동한다
  (UPDATE 기반이라 chunks.id·임베딩 rowid 보존). upsertChunk도 UPDATE-first로 변경해
